...
def load_train(obj):
    ...
    #labelPath = os.path.join("../", "data/Weibo/weibo_id_label.txt")
    labelPath = os.path.join(cwd, "data/Weibo/train_label_file.txt")
    print("loading training label:")
    F, T = [], []  # F: The false label samples' eid; T: The true label samples' eid
    l1 = l2 = 0
    labelDic = {}
    for line in open(labelPath):
        line = line.rstrip()
    ...
    print(len(labelDic))
    print(l1, l2)
    #print(len(F+T), len(random.shuffle(F+T)))
    #Res = F+F+T
    Res = F + T
    random.shuffle(Res)
...
def load_test(obj):
    ...
    labelPath = os.path.join(cwd, "data/Weibo/test_label_file.txt")
    #labelPath = os.path.join("../", "data/Weibo/weibo_id_label.txt")
